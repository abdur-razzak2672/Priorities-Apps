...
const setupModelPublicDataScore = (model, text, results) => {
  ...
  if (!model.data.moderation) model.set("data.moderation", {});
    ...
    [
      (parallelCallback) => {
        const req = {
          query: {
            textType:
              collectionType === "community" ? "communityName" : "groupName",
            targetLanguage: "en",
          },
        };
    ...
    (error) => {
      if (postName && (postDescription || postStructuredAnswers)) {
        callback(
          error,
          `${postName.content} ${
            postDescription ? postDescription.content : ""
          } ${postStructuredAnswers} ${
            postTranscript ? postTranscript.content : ""
          }`
        );
      } else {
...
const estimateToxicityScoreForCollection = (options, callback) => {
  if (process.env.GOOGLE_PERSPECTIVE_API_KEY) {
    ...
    const model =
      options.collectionType === "community" ? models.Community : models.Group;
    ...
    model
      ...
      .then((collection) => {
        if (collection) {
          ...
          if (textContent !== "") {
            log.info(
              "getToxicityScoreForText collection getting translated text"
            );
            getTranslatedTextForCollection(
              ...
              (error, translatedText) => {
                log.info(
                  "getToxicityScoreForText collection got translated text",
                  { translatedText, error }
                );
                if (error) callback(error);
                else textUsed = translatedText;
          ...
          log.error(
            "getToxicityScoreForText collection could not find collection"
          );
  ...
  if (process.env.GOOGLE_PERSPECTIVE_API_KEY) {
    ...
    models.Post.unscoped()
      ...
      .then((post) => {
        if (post) {
          let doNotStoreValue =
            post.Group.access === 0 && post.Group.Community.access === 0
              ? false
              : true;
          if (
            post.User.age_group &&
        ...
        if (point) {
          ...
          if (textContent && textContent !== "") {
            ...
            getTranslatedTextForPoint(point, (error, translatedText) => {
              ...
              else textUsed = translatedText.content;
              getToxicityScoreForText(
                textUsed,
                doNotStoreValue,
                (error, results) => {
...
const sendPostAnalyticsEvent = (post) => {
  try {
    if (
      ...
      queue.add(
        "delayed-job",
        { type: "create-activity-from-app", workData: analyticsEvent },
        "medium"
      );
    }
  } catch (error) {
