...
const renderTemplateWithLocals = (templatePath, locals, done) => {
  ...
  async.parallel([
    (callback) => {
      ejs.renderFile(
        path.join(templatePath, "html.ejs"),
        locals,
        {},
        (err, rendered) => {
          if (err) {
    ...
    (callback) => {
      ejs.renderFile(
        path.join(templatePath, "text.ejs"),
        locals,
        {},
        (err, rendered) => {
          if (err) {
...
const processNotification = (
  ...
  var method =
    user.notifications_settings[notification.from_notification_setting].method;
  var frequency =
    user.notifications_settings[notification.from_notification_setting]
      .frequency;
  ...
  if (user.email) {
    if (method !== models.AcNotification.METHOD_MUTED) {
      if (frequency === models.AcNotification.FREQUENCY_AS_IT_HAPPENS) {
        if (
          (user.email && user.email.indexOf("_anonymous@citizens.is") > -1) ||
          ...
          notification.AcActivities[0].Point &&
          ...
          log.info(
            "Email for hidden_public_group_for_domain_level_points not queued",
            { email: user.email }
          );
          callback();
        } else {
...
const filterNotificationForDelivery = function (
  notification,
  user,
  template,
  subject,
...
var sendOneEmail = function (emailLocals, callback) {
  if (
    ...
    if (
      process.env.EMAIL_CONFIG_FROM_ADDRESS &&
    ...
    if (!emailLocals.isReportingContent) emailLocals.isReportingContent = false;
    if (!emailLocals.isAutomated) emailLocals.isAutomated = false;
    if (!emailLocals.isAutomatedVision) emailLocals.isAutomatedVision = false;
      ...
      async.series(
        [
          ...
          function (seriesCallback) {
            ...
            if (
              emailLocals.domain.domain_name.indexOf("betrireykjavik.is") > -1
            ) {
              fromEmail = "Betri Reykjavík <betrireykjavik@ibuar.is>";
            } else if (
              emailLocals.domain.domain_name.indexOf("betraisland.is") > -1
            ) {
              fromEmail = "Betra Ísland <betraisland@ibuar.is>";
            } else if (
              emailLocals.domain.domain_name.indexOf("forbrukerradet.no") > -1
            ) {
              fromEmail =
                "Mine idéer Forbrukerrådet <mineideer@forbrukerradet.no>";
            } else if (
              emailLocals.domain.domain_name.indexOf("multicitychallenge.org") >
              -1
            ) {
              fromEmail = "Admins GovLab <admins@thegovlab.org>";
            } else if (
              emailLocals.domain.domain_name.indexOf("mycitychallenge.org") > -1
            ) {
              fromEmail = "Admins GovLab <admins@thegovlab.org>";
            } else if (
              emailLocals.domain.domain_name.indexOf("engagebritain.org") > -1
            ) {
              fromEmail = "Engage Britain <healthandcare@engagebritain.org>";
            } else if (
              emailLocals.domain.domain_name.indexOf("tarsalgo.net") > -1
            ) {
              fromEmail = "Társalgó <tarsalgo@kofe.hu>";
            } else if (
              emailLocals.domain.domain_name.indexOf("junges.wien") > -1
            ) {
              fromEmail = "Junges Wien <junges.wien@wienxtra.at>";
            } else if (
              emailLocals.domain.domain_name.indexOf("openmontana.org") > -1
            ) {
              fromEmail = "Open Montana <openmontana@democracylab.org>";
            } else if (
              emailLocals.domain.domain_name.indexOf("e-dem.nl") > -1
            ) {
              ...
              emailLocals.domain.domain_name.indexOf("boliden.com") > -1
            ) {
              ...
              emailLocals.domain.domain_name.indexOf(
                "puttingcommunitiesfirst.org.uk"
              ) > -1
            ) {
              fromEmail =
              ...
              emailLocals.domain.domain_name.indexOf("idea-synergy.com") > -1
            ) {
              fromEmail = "ideasynergy@idea-synergy.com";
            } else if (
              emailLocals.domain.domain_name.indexOf("smarter.nj.gov") > -1
            ) {
