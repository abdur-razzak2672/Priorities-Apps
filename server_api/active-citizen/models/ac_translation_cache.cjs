...
module.exports = (sequelize, DataTypes) => {
  let AcTranslationCache = sequelize.define(
    "AcTranslationCache",
    {
    ...
    try {
      if (req.query.textType == "aoiChoiceContent") {
        ...
        switch (req.query.textType) {
          ...
          case "pointContent":
            return modelInstance.PointRevisions[
              modelInstance.PointRevisions.length - 1
            ].content;
          ...
          case "customRatingName":
            if (
              modelInstance.Group.configuration.customRatings &&
              modelInstance.Group.configuration.customRatings[
                modelInstance.custom_rating_index
              ]
            ) {
              return modelInstance.Group.configuration.customRatings[
                modelInstance.custom_rating_index
              ].name;
            } else {
              return "No translation";
  ...
  AcTranslationCache.getSurveyAnswerTranslations = async (
    postId,
    targetLanguage,
    done
  ) => {
  ...
  AcTranslationCache.addSubOptionsToTranslationStrings = (
    ...
    if (question.radioButtons && question.radioButtons.length > 0) {
      return AcTranslationCache.addSubOptionsElements(
        textStrings,
        combinedText,
  ...
  AcTranslationCache.getRegistrationQuestionTranslations = async (
    groupId,
    targetLanguage,
    done
  ) => {
  ...
  AcTranslationCache.getSurveyQuestionTranslations = async (
    groupId,
    targetLanguage,
    done
  ) => {
    ...
    sequelize.models.AcTranslationCache.findOne({
      ...
      .then(async (translationModel) => {
        if (translationModel) {
          ...
          try {
            const results =
              await AcTranslationCache.getSurveyTranslationsFromGoogle(
                textStrings,
                targetLanguage
              );
                ...
                .then((model) => {
                  if (
                    ...
                    saveLanguageToModel
                      .update({
                        language:
                          languageInfo.data.translations[0]
  ...
  AcTranslationCache.getSurveyTranslationsFromGoogle = async (
    textsToTranslate,
    targetLanguage
  ) => {
    return new Promise(async (resolve, reject) => {
      ...
      if (
        ...
        return await AcTranslationCache.getSurveyTranslationsFromLlmFallback(
          textsToTranslate,
          targetLanguage
        );
      } else {
  ...
  AcTranslationCache.getTranslationFromGoogle = async (
    textType,
    indexKey,
    contentToTranslate,
    targetLanguage,
    ...
    if (
      HAS_LLM &&
      AcTranslationCache.ypLanguages.isoCodesNotInGoogleTranslate.includes(
        targetLanguage
      )
    ) {
              ...
              .then(() => {
                if (
                  ...
                  if (textType === "postTranscriptContent") {
                    ...
                    modelInstance.set(
                      "public_data.admin_comment.language",
                      translation.detectedSourceLanguage
                    );
                  }
                  modelInstance
                  ...
                  if (
                    AcTranslationCache.allowedTextTypesForSettingLanguage.indexOf(
                      textType
                    ) > -1
                  ) {
                    modelInstance
  ...
  AcTranslationCache.getSurveyTranslationsFromLlmFallback = async (
    textsToTranslate,
    targetLanguage
  ) => {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        const translatedStrings =
          await AcTranslationCache.llmTranslation.getListTranslation(
            targetLanguage,
            textsToTranslate
          );
  ...
  AcTranslationCache.llmGoogleTranslateFallback = async (
    textType,
    indexKey,
    contentToTranslate,
    targetLanguage,
    ...
    try {
      ...
      const translatedTextData =
        await AcTranslationCache.llmTranslation.getOneTranslation(
          targetLanguage,
          contentToTranslate
        );
            ...
            if (
              ...
              modelInstance.set(
                textType === "postTranscriptContent"
                  ? "public_data.transcript.language"
                  : "public_data.admin_comment.language",
                targetLanguage // As a fallback, using targetLanguage directly
              );
  ...
  AcTranslationCache.getAoiTranslationFromLlm = async (
    textType,
    indexKey,
    contentToTranslate,
    targetLanguage,
  ...
  AcTranslationCache.getTranslation = async (req, modelInstance, callback) => {
    const contentToTranslate =
      await sequelize.models.AcTranslationCache.getContentToTranslate(
        req,
        modelInstance
      );
    if (
        ...
        .then((translationModel) => {
          if (translationModel) {
            ...
            if (
              ...
              sequelize.models.AcTranslationCache.getTranslationFromGoogle(
                textType,
                indexKey,
                contentToTranslate,
                targetLanguage,
